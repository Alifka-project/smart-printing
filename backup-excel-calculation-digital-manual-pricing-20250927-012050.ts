...
export const EXCEL_DIGITAL_CONSTANTS = {
  parentSheetCost: 1.00, // AED per parent sheet (from user example)
...
export function calculatePerClick(sheets: number, cutPcs: number, colorsF: number): number {
...
export function excelDigitalCalculation({
  ...
  allowRotate = true,
  paperCostPerSheet // Optional manual paper pricing override
}: {
  qty: number;
  piece: { w: Cm; h: Cm };
  sides: 1 | 2;
  ...
  parent?: { w: Cm; h: Cm };
  allowRotate?: boolean;
  paperCostPerSheet?: number; // Manual paper pricing override
  ...
  const calculatePrice = (sheets: number, cutPieces: number, colorsF: number, sides: number) => {
  ...
  const sheetOptions = [
    { w: 48, h: 33, label: "48×33 cm", cutPcs: 4 },
    { w: 70, h: 33, label: "70×33 cm", cutPcs: 3 }
  ];
  ...
  return sheetOptions.map(option => {
    ...
    const effectivePaperCost = paperCostPerSheet !== undefined && paperCostPerSheet !== null 
      ? paperCostPerSheet 
      : EXCEL_DIGITAL_CONSTANTS.parentSheetCost;
