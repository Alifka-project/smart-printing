...
const Step2CustomerDetail: FC<Step2Props> = ({ formData, setFormData }) => {
  ...
  // Handle adding new customer when no existing match is found
  const handleAddNewCustomer = (fieldType: 'company' | 'firstName' | 'email', searchTerm: string) => {
    console.log(`Adding new customer for ${fieldType}:`, searchTerm);
    
    if (fieldType === 'company') {
      setClient({ companyName: searchTerm });
      setCompanySearchTerm("");
      setShowCompanySuggestions(false);
      setIsNewCustomer(true);
    } else if (fieldType === 'firstName') {
      setClient({ firstName: searchTerm });
                ...
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  {isLoadingCustomers ? (
                    ...
                    <div className="px-4 py-3">
                      ...
                      <button
                        onClick={() => handleAddNewCustomer('company', companySearchTerm)}
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    {isLoadingCustomers ? (
                      ...
                      <div className="px-4 py-3">
                        ...
                        <button
                          onClick={() => handleAddNewCustomer('email', emailSearchTerm)}
                          className="w-full flex items-center justify-center px-4 py-2 bg-[#27aae1] hover:bg-[#1e8bc3] text-white rounded-lg transition-colors duration-200 text-sm font-medium"
                        >
                          <Plus className="w-4 h-4 mr-2" />
                          Add New Email
