...
function drawCutView(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, 
  ...
  // Use specific cutting logic for 100x70 sheets instead of generic calculation
  let piecesPerRow, piecesPerCol, totalPieces;
  
  // Dynamic cutting calculation based on press dimensions
  // Calculate how many press pieces fit in parent sheet
  piecesPerRow = Math.floor(parentWidth / pressWidth);
  piecesPerCol = Math.floor(parentHeight / pressHeight);
  ...
  // Dynamic cut lines based on calculated pieces
  // Vertical cut lines
...
function calculateCutStrategy(sheetWidth: number, sheetHeight: number, maxWidth: number, maxHeight: number) {
  ...
  // For 100x70 sheets, cut into 20x14 pieces (4 pieces total)
  // This is the updated calculation as requested
  let pieceWidth, pieceHeight;
  
  if (sheetWidth === 100 && sheetHeight === 70) {
    // Special case: 100x70 sheet should be cut into 20x14 pieces
    pieceWidth = 20;
    pieceHeight = 14;
  } else {
    // For other sheet sizes, use machine max dimensions
    pieceWidth = maxWidth;
  ...
  const perPaperCalc = React.useMemo(() => {
    ...
    return formData.products.map((product, productIndex) => {
      ...
      return product.papers.map((paper, paperIndex) => {
        ...
        // Use Excel-based calculation for recommended sheets
        ...
        // Count colors for this paper
        const currentPaperColors = paperColors[productIndex]?.[paperIndex] || [];
        const colors = Math.max(currentPaperColors.length, 1); // Minimum 1 color
        
        // Use Excel-based calculation
        try {
          ...
          recommendedSheets = excelResult.totalSheets;
          
          // Update layout with Excel-based pieces per sheet
          layout.itemsPerSheet = excelResult.piecesPerSheet;
          layout.efficiency = (excelResult.piecesPerSheet / (100 * 70 / (step3ProductWidth * step3ProductHeight))) * 100;
          
          console.log('📊 Excel-based calculation result:', {
            productName: product?.productName,
            piecesPerSheet: excelResult.piecesPerSheet,
            recommendedSheets: excelResult.totalSheets,
            cuttingSize: excelResult.cuttingSize,
            wasteSheets: excelResult.wasteSheets,
            totalPrice: excelResult.totalPrice
          });
        } catch (error) {
          console.error('❌ Excel calculation failed, falling back to original:', error);
                      ...
                      <canvas
                        ...
                        ref={(canvas) => {
                          if (canvas && opPaper?.inputWidth && opPaper?.inputHeight) {
                            setTimeout(() => {
                              // Use correct cutting dimensions based on sheet size
                              // For 100x70 sheets, cut into 20x14 pieces; for other sheets, use machine max
                              const machineMaxWidth = (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 20 : 52;
                              const machineMaxHeight = (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 14 : 72;
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-[#27aae1]">
                            {opPaper?.inputWidth === 100 && opPaper?.inputHeight === 70 ? "20 × 14 cm" : "52 × 72 cm"}
                          </span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    <div className="bg-white rounded-xl p-4 border border-red-200 shadow-sm">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-red-600">
                            {opPaper?.inputWidth && opPaper?.inputHeight ? 
                              calculateCutPieces(opPaper.inputWidth!, opPaper.inputHeight!, 
                                (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 20 : 52, 
                                (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 14 : 72).totalPieces : "–"}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-[#27aae1]">
                            {opPaper?.inputWidth && opPaper?.inputHeight ? 
                              (() => {
                                const cutPieces = calculateCutPieces(opPaper.inputWidth!, opPaper.inputHeight!, 
                                  (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 20 : 52, 
                                  (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 14 : 72);
                            ...
                            {opPaper?.inputWidth && opPaper?.inputHeight ? 
                              (() => {
                                const cutPieces = calculateCutPieces(opPaper.inputWidth!, opPaper.inputHeight!, 
                                  (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 20 : 52, 
                                  (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 14 : 72);
                                const cutSize = (opPaper.inputWidth === 100 && opPaper.inputHeight === 70) ? 20 * 14 : 52 * 72;
      ...
      <Dialog open={showFinishingDetails} onOpenChange={setShowFinishingDetails}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {formData.operational?.finishing?.map((finish, index) => (
                <div key={index} className="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      <h4 className="text-lg font-semibold text-slate-800">{finish.name || `Finishing ${index + 1}`}</h4>
                      <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">
                        Both
                      </span>
                    </div>
                    <span className="text-xl font-bold text-slate-800">{fmt(finish.cost || 0)}</span>
                  </div>
                  
                  <div className="space-y-2 text-sm text-slate-600">
                    <p>Auto-calculated based on finishing type and quantity</p>
                    <div className="bg-slate-50 rounded p-2 font-mono text-xs">
                      Formula: {finish.name} × quantity = {fmt(finish.cost || 0)}
                    </div>
                  </div>
                </div>
              )) || (
