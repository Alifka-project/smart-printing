    ...
    <div className="space-y-6 md:space-y-8 px-4 md:px-0">
                        ...
                        {(() => {
                          try {
                            ...
                            const excelResult = calculateExcelBasedPricing({
                              ...
                              productHeight: outputDimensions[productIndex]?.height || product?.flatSize?.height || 5.5,
                              quantity: qty,
                              paperCostPerSheet: opPaper?.pricePerSheet != null 
                                ? opPaper.pricePerSheet
                                : (opPaper?.pricePerPacket != null && opPaper?.sheetsPerPacket != null && opPaper.sheetsPerPacket > 0
                                  ? opPaper.pricePerPacket / opPaper.sheetsPerPacket
                                  : 1),
                  ...
                  id="unified-canvas-visualization"
                    ...
                    if (canvas && formData.products.length > 0 && formData.operational.papers.length > 0) {
                      ...
                      if (firstLayout && firstLayout.layout && firstLayout.layout.itemsPerSheet > 0 && firstPaper?.inputWidth && firstPaper?.inputHeight) {
                            setTimeout(() => {
                          // Use Excel-based calculation results for visualization
                          ...
                          // Get Excel-based calculation results
                          try {
                            const excelResult = calculateExcelBasedPricing({
                              productWidth: step3ProductWidth,
                              productHeight: step3ProductHeight,
                              quantity: firstProduct?.quantity || 1000,
                              paperCostPerSheet: firstPaper?.pricePerSheet || 5,
                              colors: Math.max(paperColors[0]?.[0]?.length || 1, 1),
                              sides: firstProduct?.sides || '2',
                              printingSelection: firstProduct?.printingSelection || 'Offset',
                              parentSheetWidth: firstPaper?.inputWidth || 100,
                              parentSheetHeight: firstPaper?.inputHeight || 70
                            });
                            
                            // Use Excel-based cutting size and pieces per sheet
                            const excelCuttingWidth = excelResult.cuttingSize.width;
                            const excelCuttingHeight = excelResult.cuttingSize.height;
                            const excelPiecesPerSheet = excelResult.piecesPerSheet;
                            
                            // Calculate how many cutting papers fit on parent sheet
                            const parentWidth = firstPaper.inputWidth || 100;
                            const parentHeight = firstPaper.inputHeight || 70;
                            const cuttingPapersPerRow = Math.floor(parentWidth / excelCuttingWidth);
                            const cuttingPapersPerCol = Math.floor(parentHeight / excelCuttingHeight);
                            const totalCuttingPapers = cuttingPapersPerRow * cuttingPapersPerCol;
                            
                            // Create modified layout with Excel results
                            const excelLayout = {
                              ...firstLayout.layout,
                              itemsPerSheet: excelPiecesPerSheet,
                              itemsPerRow: Math.ceil(Math.sqrt(excelPiecesPerSheet)),
                              itemsPerCol: Math.ceil(excelPiecesPerSheet / Math.ceil(Math.sqrt(excelPiecesPerSheet))),
                              usableW: excelCuttingWidth,
                              usableH: excelCuttingHeight,
                              // Add cutting paper layout info
                              cuttingPapersPerRow,
                              cuttingPapersPerCol,
                              totalCuttingPapers
                            };
                            ...
                            console.log('🎨 Using Excel-based visualization data:', {
                              productName,
                              originalLayout: firstLayout.layout,
                              excelResult: {
                                cuttingSize: excelResult.cuttingSize,
                                piecesPerSheet: excelResult.piecesPerSheet,
                              ...
                              cuttingPaperLayout: {
                                papersPerRow: cuttingPapersPerRow,
                                papersPerCol: cuttingPapersPerCol,
                                totalPapers: totalCuttingPapers
                              },
                              excelLayout
                            });
                            ...
                            drawProfessionalVisualization(
                              ...
                              settings, 
                              firstProduct,
                              firstPaper.inputWidth,  // Parent sheet width (100)
                              firstPaper.inputHeight, // Parent sheet height (70)
                              excelCuttingWidth,      // Press sheet width (20) - Excel cutting size
                              excelCuttingHeight,     // Press sheet height (14) - Excel cutting size
                              formData,
                              0
                            );
                          } catch (error) {
                            console.error('❌ Excel visualization calculation failed, using original layout:', error);
                            ...
                            const settings = {
                              ...
                              firstLayout.layout, 
                                  ...
                                  settings, 
                              firstProduct,
                              firstPaper.inputWidth,
                              firstPaper.inputHeight,
                              step3ProductWidth,
                              step3ProductHeight,
                              formData,
                              0
      ...
      <Dialog open={showCostBreakdown} onOpenChange={setShowCostBreakdown}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            {/* Cost breakdown content will be added here */}
            <div className="text-center py-8">
              <div className="text-slate-500">Cost breakdown functionality will be implemented here</div>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-blue-800">{fmt(calculateTotalCost(openData, openData.quantity || 0, openData.quantity || 0, openData.quantity || 0))}</span>
                      </div>
                        <div className="flex justify-between">
                          <span className="text-blue-700">Additional Costs:</span>
                          <span className="font-semibold text-blue-800">{fmt((formData.additionalCosts || []).reduce((acc, f) => acc + (f.cost || 0), 0))}</span>
                      </div>
                    </div>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between border-t pt-2">
                          ...
                          <span className="text-xl font-bold text-blue-900">{fmt(calculateTotalCost(openData, openData.quantity || 0, openData.quantity || 0, openData.quantity || 0) + (formData.additionalCosts || []).reduce((acc, f) => acc + (f.cost || 0), 0) + calculateFinishingCosts())}</span>
                </div>
              </div>
            </div>
                        </div>
      ...
      <Dialog open={showPricingLogic} onOpenChange={setShowPricingLogic}>
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-slate-600 mt-2">Understanding our pricing calculation methodology</p>
          </DialogHeader>
          
          <div className="space-y-6">
            <div className="text-center py-8">
              <div className="text-slate-500">Pricing logic explanation will be implemented here</div>
            </div>
          </div>
      ...
      <Dialog open={showPaperPrice !== null} onOpenChange={() => setShowPaperPrice(null)}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-slate-600 mt-2">Detailed paper pricing breakdown</p>
          </DialogHeader>
            ...
            {(() => {
              ...
              if (!paperData) return <div>No paper data available</div>;
              
              return (
                  <div className="space-y-4">
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    <h4 className="text-lg font-semibold text-blue-800 mb-3">Paper Details</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-2">
                          <div className="flex justify-between">
                          <span className="text-blue-700">Size:</span>
                          <span className="font-semibold text-blue-800">{paperData.inputWidth} × {paperData.inputHeight} cm</span>
                          </div>
                          <div className="flex justify-between">
                          <span className="text-blue-700">Price per Sheet:</span>
                          <span className="font-semibold text-blue-800">{paperData.pricePerSheet ? `${paperData.pricePerSheet} AED` : 'Not set'}</span>
                          </div>
                            </div>
                        <div className="space-y-2">
                          <div className="flex justify-between">
                          <span className="text-blue-700">Price per Packet:</span>
                          <span className="font-semibold text-blue-800">{paperData.pricePerPacket ? `${paperData.pricePerPacket} AED` : 'Not set'}</span>
                          </div>
                          <div className="flex justify-between">
      ...
      {formData.products.length > 0 && (
        <Card className="border-0 shadow-lg w-full">
          ...
          <CardContent className="space-y-4">
            <div className="text-center py-8">
              <div className="text-slate-500">Costing analysis functionality will be implemented here</div>
              </div>
          </CardContent>
        </Card>
      )}
        ...
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center text-2xl font-bold text-slate-800">
              ...
              Finishing Details & Cost Breakdown
            </DialogTitle>
          ...
          <div className="space-y-6">
            <div className="text-center py-8">
              <div className="text-slate-500">Finishing details functionality will be implemented here</div>
            </div>
          </div>
