...
const Step5Quotation: React.FC<Step5Props> = ({
  ...
  // Calculate total costs using the same logic as Step 4 (single calculation for all products)
  const calculateTotalCosts = () => {
    // Use the same calculation logic as Step 4 from the main page
    ...
    const finishingCost = formData.operational.finishing.reduce((total, f) => {
      // Check if this finishing is used by any product
    ...
    // 4. Calculate base price (same as Step 4)
    const basePrice = paperCost + platesCost + finishingCost;

    // 5. Add additional costs to base price (same as Step 4)
    ...
    // 6. Apply margin and VAT (same as Step 4)
    const MARGIN_PERCENTAGE = 0.30; // 30% margin
    const VAT_PERCENTAGE = 0.05; // 5% VAT
  ...
  // Calculate per-product costs by distributing the total proportionally
  const calculateProductCosts = (productIndex: number) => {
    ...
    // Get total costs using Step 4 logic
    ...
    // Calculate this product's share based on quantity
    const quantityRatio = totalQuantity > 0 ? (product.quantity || 0) / totalQuantity : 0;

    return {
      paperCost: totalCosts.paperCost * quantityRatio,
      platesCost: totalCosts.platesCost * quantityRatio,
      finishingCost: totalCosts.finishingCost * quantityRatio,
      subtotal: totalCosts.basePrice * quantityRatio,
      margin: 30, // 30% margin
      marginAmount: totalCosts.marginAmount * quantityRatio,
      vat: totalCosts.vatAmount * quantityRatio,
      total: totalCosts.totalPrice * quantityRatio
    };
  };
  ...
  const calculateSummaryTotals = () => {
    // Use the same calculation logic as Step 4 (single calculation for all products)
    const totalCosts = calculateTotalCosts();
    
    // Add supplementary quantities (proportionally calculated)
    let supplementaryTotal = 0;
    otherQuantities.forEach((otherQty) => {
      const prices = calculateOtherQtyPrice(otherQty);
      supplementaryTotal += prices.total;
    });

    // Calculate grand total before discount
    const grandTotal = totalCosts.totalPrice + supplementaryTotal;
    ...
    return {
      totalPaperCost: totalCosts.paperCost,
      totalPlatesCost: totalCosts.platesCost,
      totalFinishingCost: totalCosts.finishingCost,
      totalAdditionalCost: totalCosts.additionalCostsTotal,
      totalSubtotal: totalCosts.basePrice, // Base cost before margin/VAT
      totalMargin: 30, // 30% margin (hidden from user)
      totalMarginAmount: totalCosts.marginAmount,
      totalVAT: totalCosts.vatAmount,
      grandTotal,
      discountAmount,
      finalTotal
    };
