                  ...
                  <div className="space-y-4">
                    {tableColumns.map((column) => (
                      <div key={column.name} className="space-y-1">
                        <div className="flex items-center space-x-2">
                          <span className="text-xs font-medium text-slate-500 uppercase tracking-wide">{column.name}</span>
                          {column.isPrimary && (
                            <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">PK</span>
                          )}
                          {column.isRequired && (
                            <span className="text-xs bg-red-100 text-red-800 px-2 py-0.5 rounded-full">*</span>
                          )}
                        </div>
                        <div className="text-sm text-slate-900 break-all font-medium">
                          {row[column.name]?.toString() || '—'}
                        </div>
                      </div>
                    ))}
